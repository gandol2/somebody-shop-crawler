...
import { getLastUpdatedChannel, updateChannel, updateChannelUpdateDay, } from "../libs/db/channelHelper.js";
...
async function main() {
    ...
    if (!channel) {
        printError("DB에 등록된 채널이 없습니다. 채널정보를 등록 후 이용 해주세요.");
    ...
    try {
        if (!channel.no) {
            //! 스토어NO가 없는경우
            printInfo(`신규채널 - 채널NO가 없습니다. 채널정보를 가져오는중 (채널URL:${channel.url})`);
            //* 스마트스토어 정보 API 호출
            ...
            printLog(`신규채널 - 수집 완료 (채널URL:${channel.url} / 채널NO:${channel.no} / 채널이름:${channel.name})`);
        }
        //^ ========================================= 상품수집
        if (channel && channel.id && channel.no) {
            ...
            if (수집일자주기_DAY > day) {
                printInfo(`상품수집❌ - ${day}일전 상품 수집이 완료된 채널 입니다. (설정된 수집주기:${수집일자주기_DAY}일)`);
                printInfo("프로그램을 종료 합니다.");
                return;
            }
            const productResponse = await getProducts(channel.no, "POPULAR", 10);
            const categorys = productResponse.simpleProducts.map((ele) => ele.category);
            //* 카테고리 목록 업데이트
            ...
            await sleep(1000);
            printInfo(`상품수집 - 시작 (${channel.name}:https://m.smartstore.naver.com/${channel.url})`);
            productResponse.simpleProducts.forEach(async (product, idx, all) => {
                ...
                const result = {
                    ...
                    channelNo: channel === null || channel === void 0 ? void 0 : channel.no,
                    ...
                    url: `https://m.smartstore.naver.com/${channel === null || channel === void 0 ? void 0 : channel.url}/products/${product.id}`,
                };
                ...
                if (!categorie) {
                    categorie = await prisma.category.create({
                        data: Object.assign({}, product.category),
                    });
                }
                    ...
                    update: {
                        ...
                        sales: {
                            create: {
                                ...
                                price: price,
                                saleCount6M: saleCount6M,
                                saleAmount6M: saleAmount6M,
                                saleCount3D: saleCount3D,
                                saleAmount3D: saleAmount3D,
                                salCount6MAvg1D: parseFloat(salCount6MAvg1D.toFixed(2)),
                                salAmount6MAvg1D: salAmount6MAvg1D,
                                salCount3DAvg1D: parseFloat(salCount3DAvg1D.toFixed(2)),
                                salAmount3DAvg1D: salAmount3DAvg1D,
...
main()
    .then(async () => {
    printInfo(`상품수집 - 완료 ${0 < salesResult.length ? `(${salesResult.length}개)` : `(고매출 상품❌)`}`);
    await serverPing();
    if (0 < salesResult.length) {
        // if (true) {
        salesResult.forEach((ele) => printYellow(`찾은상품 - 매출(1일): ${ele.salAmount3DAvg1D.toLocaleString()}원 / ${ele.productName}:${ele.url}`));
    }
})
    .catch(async (e) => {
    printError(`오류발생`);
    ...
    printLog(`----------------------------------------------------- (실행시간 : ${end.diff(start, "second")}초)`);
